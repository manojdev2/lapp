...
/*
 * Igual WP Framework Class
 */

class Igual_Wp_Framework {
	
	private static $_instance = null;

	public function __construct(){
		//Search form filter
		add_filter( 'get_search_form', array( 'Igual_Wp_Framework', 'igual_search_form' ) );
	}
	
	public static function igual_mobile_logo() {
		...
		$logo_values = Igual_Wp_Elements::igual_get_meta_and_option_values( $logo_keys );
		extract( $logo_values );
	...
	public static function igual_site_logo( $sticky = false ) {
		$site_logo = '';
		...
		if( !empty( $site_logo ) && is_array( $site_logo ) && isset( $site_logo['image'] ) ){
			...
			if ( wp_attachment_is_image( $site_logo_id ) ) {
				...
				if( !$sticky ) {
					Igual_Wp_Framework::igual_basic_logo();
				}
			}
		}
	}
	
	public static function igual_sticky_logo() {
		...
		if( !empty( $sticky_logo ) && is_array( $sticky_logo ) && isset( $sticky_logo['image'] ) ){
			...
			if ( wp_attachment_is_image( $sticky_logo_id ) ) {
				...
				Igual_Wp_Framework::igual_site_logo(true);
			}
		}
	}

	public static function igual_basic_logo( $args = array(), $head_tag = 'h1' ) {
		$site_title = get_bloginfo( 'name' );
		...
		$contents   = '';
		$classname  = '';
		//$page_title_opts = Igual_Wp_Elements::igual_options( 'page-title-enable' );
		$current = '';
		...
		$args = apply_filters( 'igual_site_logo_args', $args, $defaults );
		if ( has_custom_logo() ) {
		...
		echo apply_filters( 'igual_site_logo', $html, $args, $classname, $contents );
	}

	public static function igual_site_description() { //Igual_Wp_Framework::igual_site_description()
		$logo_keys = array(
			'chk' => 'logo-chk',
			'fields' => array(
		...
		if( isset( $logo_desc ) && $logo_desc ){
			...
			echo apply_filters( 'igual_site_description', $html, $description, $wrapper );
		}
	}

	public static function igual_get_the_terms_as_out( $post_id, $term_slug ){ //Igual_Wp_Framework::igual_get_the_terms_as_out
	...
	public static function igual_get_post_date_as_link(){ //Igual_Wp_Framework::igual_get_post_date_as_link
	...
	public static function igual_get_post_author(){ //Igual_Wp_Framework::igual_get_post_author
	...
	public static function igual_get_post_comment_count(){ //Igual_Wp_Framework::igual_get_post_comment_count
		if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="bi bi-chat-left-dots"></span> ';
			comments_popup_link( esc_html__( 'No Comments', 'igual' ), esc_html__( '1 Comment', 'igual' ), esc_html__( '%s Comments', 'igual' ) );
		}
	}
	
	public static function igual_email_link( $email, $target = '_self' ){ 
		if( $email )
		echo '<a href="mailto:'. esc_attr( $email ) .'" class="email-link" target="'. esc_attr( $target ) .'">'. esc_html( $email ) .'</a>';
	}
	
	public static function igual_address( $address ){ 
		if( $address ){
			echo '<address>'. nl2br( $address ) .'</address>';
		}
	}

	public static function igual_search_form(){ 
		ob_start();	
	?>
	...
	public static function igual_fullbar_search_form(){ ?>
	...
	public static function igual_overlay_search_form(){
	?>
	...
	public static function igual_textbox_search_form(){
	?>		
		<div class="textbox-search-wrap">
			<form method="get" class="search-form" action="<?php echo esc_url( home_url( '/' ) ); ?>">
				<div class="input-group">
					<input name="s" type="text" class="form-control" value="<?php echo get_search_query(); ?>" placeholder="<?php echo apply_filters( 'igual_search_placeholder', esc_attr__( 'Search and hit enter..', 'igual' ) ); ?>">
				</div>
			</form>
		</div>		
	<?php
	...
	public static function igual_search_modal( $serach_opt, $part = 'navbar' ){
		switch( $serach_opt ){
		
			case '1': ?>
				<a class="full-search-toggle" href="#"><i class="bi bi-search"></i></a>
			<?php
				add_action( 'igual_footer_after', array( 'Igual_Wp_Framework', 'igual_overlay_search_form' ), 10 );
			break;
			
			case '2': 
				self::igual_textbox_search_form();
			?>				
			...
			<?php
				add_action( 'igual_'. esc_attr( $part ) .'_after', array( 'Igual_Wp_Framework' , "igual_fullbar_search_form" ) , 10 );
			break;
...
Igual_Wp_Framework::get_instance();