...
/*
 * Igual WP Elements Class
 */

class Igual_Wp_Elements {
	...
	public static $igual_page_options = null;
	...
	public function __construct() {
		...
		//Get theme option
		self::$igual_options = class_exists( 'Igual_Theme_Option' ) ? Igual_Theme_Option::$igual_options : apply_filters( 'igual_options', get_option( 'igual_options' ) );
		
		if( function_exists( 'igual_get_woo_merge_config' ) ){
			if( !isset( self::$igual_options['shop-title'] ) ) {
				self::$igual_options = igual_get_woo_merge_config( self::$igual_options );
			}
		}
		
		//Back to top
		add_action( 'igual_footer_after', array( $this, 'igual_back_to_top' ), 40 );
		
	}

	public static function igual_options( $element ){ //Igual_Wp_Elements::igual_options()
		$opt_array = self::$igual_options;
		return !empty( $element ) && isset( $opt_array[$element] ) ? $opt_array[$element] : '';
	}
	
	public static function igual_get_meta_and_option_values( $keys ){
		$post_options = self::$igual_page_options;
		...
		if( empty( $post_options ) ){
			...
			foreach( $keys['fields'] as $field => $key ) {
				...
				if( $count > 1 ){
					$options[$field] = isset( $igual_options[$key[1]] ) ? $igual_options[$key[1]] : '';					
				}elseif( isset( $igual_options[$key] ) ){
					$options[$field] = isset( $igual_options[$key] ) ? $igual_options[$key] : '';
				}
			}
		}
	...
	public static function igual_get_content_class(){
	...
	public static function igual_get_template_sidebars( $page_title_values, $side, $col_class = '' ){
	...
	public static function igual_show_page_title( $title_items ){
		...
		if ( is_singular() ) {
			...
			if ( $wp_query->found_posts ) {
				...
				$archive_subtitle = esc_html__( 'We could not find any results for your search. You can give it another try through the search form below.', 'igual' );
			}
		}elseif ( is_home() ) {
			$archive_title    = self::igual_options( 'blog-page-title' );
			$archive_subtitle = self::igual_options( 'blog-page-description' );
		}elseif ( is_404() ) {
			$archive_title    = esc_html__( 'Error 404', 'igual' );
		}else {
			$archive_title    = get_the_archive_title();
			$archive_subtitle = get_the_archive_description();
		}

		$archive_title = apply_filters( 'igual_page_title', $archive_title );
		$archive_subtitle = apply_filters( 'igual_page_title_description', $archive_subtitle );
	...
	public static function igual_get_post_meta( $template = 'single', $part = 'left' ){
		$meta_items = Igual_Wp_Elements::igual_options( esc_attr( $template ) .'-'. esc_attr( $part ) .'-meta-items');
		if( !empty( $meta_items ) ):	
				...
				foreach( $meta_items as $key => $value ){
					if( !empty( $value ) ):
						...
						foreach( $value as $element => $label ){
							self::igual_post_meta_items($element);
						}
						echo '</ul>';
					endif;
				}
	...
	public static function igual_post_meta_items( $element = '' ){
		switch($element){
			case "date": ?>
				<li class="post-date"><?php Igual_Wp_Framework::igual_get_post_date_as_link(); ?></li>
			<?php 
			break;
			case "author": ?>
				<li class="post-author"><?php Igual_Wp_Framework::igual_get_post_author(); ?></li>
			<?php 
			break;
			case "category": 
			...
			?>
				<li class="post-category"> <?php Igual_Wp_Framework::igual_get_the_terms_as_out( get_the_ID(), 'category' ); ?></li>
			<?php
				endif; 
			break;
			case "tag": 
				...
				if ( $terms && ! is_wp_error( $terms ) ) :
			?>
				<li class="post-tag"><span class="bi bi-tag"></span> <?php Igual_Wp_Framework::igual_get_the_terms_as_out( get_the_ID(), 'post_tag' ); ?></li>
			<?php 
				endif;
			break;
			case "more": if( !is_singular() ): ?>
				<li class="post-more pull-right"><a href="<?php echo esc_url( get_the_permalink() ); ?>"><?php esc_html_e( 'Read more', 'igual' ); ?></a></li>
			<?php 
				endif;
			break;
			case "share": 
			...
			?>
				<li class="post-share-wrap"><?php Igual_Custom_Functions::igual_social_share(); ?></li>
			<?php
				endif;
			break;
		}
	...
	public static function igual_values_to_currency_format( $value ){
		$result = '';
	...
	public static function igual_breadcrumbs() { //Igual_Wp_Elements::igual_breadcrumbs()	 
		$showOnHome = 1; // 1 - show breadcrumbs on the homepage, 0 - don't show
		$delimiter = '<i class="breadcrumb-delimiter"></i>';//'&raquo;'; // delimiter between crumbs
		$home = esc_html__( 'Home', 'igual' ); // text for the 'Home' link
		...
		if (is_home() || is_front_page()) {
			...
			if ( !is_single() && !is_page() && get_post_type() != 'post' && !is_404() ) {
				...
				if ($thisCat->parent != 0){
					$bread_out .= self::igual_get_term_parents_list( $thisCat, 'category', array( 'separator' => ' ' . $delimiter . ' ' ) );
				}
				...
				if ( get_post_type() != 'post' ) {
					...
					$cats = self::igual_get_term_parents_list( $cat, 'category', array( 'separator' => ' ' . $delimiter . ' ' ) );
				...
				$bread_out .= wp_kses( $before . esc_html__( 'Posts by ', 'igual' ) . $userdata->display_name . $after, $allowed_html );
			} elseif ( is_404() ) {
				$bread_out .= wp_kses( $before . esc_html__( 'Error 404', 'igual' ) . $after, $allowed_html );
			}
			if ( get_query_var('paged') ) {
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) $bread_out .= ' (';
				$bread_out .= esc_html__( 'Page', 'igual' ) . ' ' . get_query_var('paged');
				if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) $bread_out .= ')';
			}
		}
		$bread_out .= '</ul></li>';
	...
	public static function igual_get_term_parents_list( $term_id, $taxonomy, $args = array() ) {
		$list = '';
	...
	public static function igual_secondary_bar(){
		$secondary_bar = Igual_Wp_Elements::igual_options('secondary-sidebar');
		if( !empty( $secondary_bar ) && is_active_sidebar( $secondary_bar ) ){ 
			$animation_from = Igual_Wp_Elements::igual_options('secondary-sidebar-from');
	...
	public static function igual_back_to_top(){ ?>
		<a href="<?php echo esc_url( site_url() ); ?>" class="back-to-top" id="back-to-top"><i class="bi bi-caret-up-fill"></i><?php esc_html_e( 'To Top', 'igual' ); ?></a>
	<?php
	}
...
Igual_Wp_Elements::get_instance();