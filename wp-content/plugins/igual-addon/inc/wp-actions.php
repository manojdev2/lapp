...
class Igual_Wp_Actions {
	...
	public function __construct() {
		...
		//Google fonts
		add_action( 'wp_head', array( $this, 'igual_google_fonts_con' ), 10 );

		//Theme front scripts
		add_action( 'wp_enqueue_scripts', array( $this, 'igual_front_scripts' ), 10 );

		//body class
		add_filter( 'body_class', array( $this, 'igual_custom_class' ), 99 );

	}

	public static function igual_google_fonts_con(){

		$g_arr = get_option( 'igual_google_fonts_list' );
	...
	function igual_front_scripts(){
		//Register font awesome styles
		wp_register_style( 'font-awesome', IGUAL_ADDON_URL . 'admin/extension/theme-options/assets/css/font-awesome.min.css', array(), '4.7.0', 'all' );
	}

	function igual_custom_class( $classes ) {
		$igual_options = get_option( 'igual_options' ); 
		$rtl = isset( $igual_options['rtl'] ) ? $igual_options['rtl'] : false;
		if( $rtl ) {
			$classes[] = 'rtl';
		}
