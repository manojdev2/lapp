<?php
class Igual_Mailchimp_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array( 'classname' => 'igual_mailchimp_widget', 'description' => esc_html__('Mailchimp Widget', 'igual-addon') );
		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'igual_mailchimp_widget' );
		
		$this->default_failure_message = esc_html__('There was a problem processing your submission.', 'igual-addon');
		$this->default_signup_text = esc_html__('Join now!', 'igual-addon');
		$this->default_success_message = esc_html__('Thank you for joining our mailing list. Please check your email for a confirmation link.', 'igual-addon');
		$this->default_title = esc_html__('Sign up for our mailing list.', 'igual-addon');
		$this->api_key = Igual_Theme_Option::igual_options( 'mailchimp-api' );
		
		/* Create the widget. */
		parent::__construct( 'igual_mailchimp_widget', esc_html__( 'Igual Mailchimp', 'igual-addon' ), $widget_ops, $control_ops );
	}
	/**
		...
		<div class="mailchimp-wrapper">
			<form class="zozo-mc-form" id="<?php echo 'zozo-mc-form'; ?>" method="post">
				...
				<div class="form-group">
					<input type="text" placeholder="<?php esc_html_e('First Name', 'igual-addon'); ?>" class="form-control first-name" name="zozo_mc_first_name" />
				</div>
				<?php
					}
				...
				?>	
				<div class="form-group">
					<input type="text" placeholder="<?php esc_html_e('Last Name', 'igual-addon'); ?>" class="form-control last-name" name="zozo_mc_last_name" />
				</div>
				<?php	
					}
				?>
				<input type="hidden" name="igual_mc_listid" value="<?php echo stripslashes( $instance['current_mailing_list'] ); ?>" />
				
				<div class="input-group">
					<input type="text" class="form-control zozo-mc-email" required="required" id="zozo-mc-email" placeholder="<?php esc_html_e('Email Address', 'igual-addon'); ?>" name="zozo_mc_email">
			...
			<?php
				$success = $instance['success_message'] && $instance['success_message'] != '' ? $instance['success_message'] : esc_html__( 'Success', 'igual-addon' );
				$fail = $instance['failure_message'] && $instance['failure_message'] != '' ? $instance['failure_message'] : esc_html__( 'Failed', 'igual-addon' );
			?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'current_mailing_list' => '', 'signup_text' => '', 'collect_first' => '', 'collect_last' => '', 'subtitle' => '', 'success_message' => esc_html__('Success.', 'igual-addon'), 'failure_message' => esc_html__('Failure.', 'igual-addon'));
		...
		?>
		<h3><?php echo  esc_html__('General Settings', 'igual-addon');?></h3>
		<!-- Widget Title: Text Input -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'zozo-mc-title' ) ); ?>"><?php esc_html_e('Title:', 'igual-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('current_mailing_list') ); ?>"><?php echo esc_html__('Select a Mailing List :', 'igual-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('signup_text') ); ?>"><?php echo esc_html__('Sign Up Button Text :', 'igual-addon'); ?></label>
		...
		<h3><?php echo esc_html__('Personal Information', 'igual-addon'); ?></h3>
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id('collect_first') ); ?>"><?php echo esc_html__('Collect first name.', 'igual-addon'); ?></label>
			<br />
			...
			<label><?php echo esc_html__('Collect last name.', 'igual-addon'); ?></label>
		</p>
		<h3><?php echo esc_html__('Notifications', 'igual-addon'); ?></h3>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('subtitle') ); ?>"><?php echo esc_html__('Sub Title:', 'igual-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('success_message') ); ?>"><?php echo esc_html__('Success Message:', 'igual-addon'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id('failure_message') ); ?>"><?php echo esc_html__('Failure Message:', 'igual-addon'); ?></label>
...
if( ! function_exists('igual_mailchimp') ) {
	function igual_mailchimp(){
				
		$nonce = $_POST['nonce'];
	  
		if ( ! wp_verify_nonce( $nonce, 'igual-mailchimp' ) )
			die ( esc_html__( 'Busted', 'igual-addon' ) );
		
		$list_id = isset( $_POST['igual_mc_listid'] ) ? $_POST['igual_mc_listid'] : '';
		...
		$success = isset( $_POST['igual_mc_success'] ) ? $_POST['igual_mc_success'] : '';
		...
		$api_key = Igual_Theme_Option::igual_options( 'mailchimp-api' );
	...
	add_action('wp_ajax_nopriv_zozo-mc', 'igual_mailchimp');
	add_action('wp_ajax_zozo-mc', 'igual_mailchimp');
}

//Mailchimp Get list id's
if( ! function_exists('igual_get_mailchimp_list_ids') ) {
	function igual_get_mailchimp_list_ids(){
