...
( function( $ ) {
	...
	$(document).on( 'click', '.button-uninstall-demo', function(e) {
		...
		$.confirm({
			theme: 'supervan',
			title: false,
			content: igual_admin_ajax_var.unins_confirm,
			confirmButtonClass: 'btn-success',
			cancelButtonClass: 'btn-danger',
			confirmButton: 'Uninstall',//igual_admin_ajax_var.yes,
   			cancelButton: igual_admin_ajax_var.no,
			confirm: function(){
				...
				$(progress).prev(".progress-text").text(igual_admin_ajax_var.uninstalling + '..');
				loading_wrap.show();
				...
				$.ajax({
					...
					url: ajaxurl,
					data: {
						action: 'igual_demo_import',
						process: 'uninstall',
						nonce:  $("#igual_demo_import_nonce").val()
					},
					success: function(response){
						if( response && response.indexOf('success') == -1 ) {
							...
							loading_wrap.hide();
							progress.parent(".installation-progress").hide();
							progress_text.text(igual_admin_ajax_var.uninstalled);
					...
					error: function(response, errorThrown){
						loading_wrap.hide();
						progress.parent(".installation-progress").hide();
						alert(igual_admin_ajax_var.unins_pbm);
					}
				});
			}
		});
			...
			$(document).on( 'click', '.button-install-demo', function(e) {
				...
				if( choosed_demo !== null ) {
					...
					$.confirm({
						...
						confirmButtonClass: 'btn-success',
    					cancelButtonClass: 'btn-danger',
						confirmButton: igual_admin_ajax_var.proceed,
   						cancelButton: igual_admin_ajax_var.cancel,
						confirm: function(){
							...
							loading_wrap.show();
							$(current).parents('.zozothemes-demo-item').find('.installation-progress p').text(igual_admin_ajax_var.downloading);
							$('.zozo-importer-notice').hide();
						
							// Demo Files Download
							igual_check_file_access_permission( choosed_demo, revslider, progress );
	...
	function igual_progress_update( progress ){
	...
	function igual_progress_end( choosed_demo, progress ){
		...
		$.confirm({
			...
			confirmButton: 'Ok',
			cancelButton: 'Close',
			confirm: function(){ window.location = igual_admin_ajax_var.igual_import_url; }
		});
	}
	
	function igual_progress_details( progress, msg, stat ){
	...
	function igual_check_file_access_permission( choosed_demo, revslider, progress ){
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: ajaxurl,
			...
			success: function(response){
				if( response['msg'] == 'failed' ) {
					...
					igual_progress_details( progress, response['msg'], '' );
					igual_progress_end( choosed_demo, progress );					
				}else{
					igual_general_file_download_function( 0, choosed_demo, revslider, progress );
				}
			},
			error: function(response, errorThrown){
				 
				igual_progress_details( progress, response['msg'], 'failed' );
				igual_progress_end( choosed_demo, progress );
			}
		});//ajax end		
	}
	
	function igual_general_file_download_function( index, choosed_demo, revslider, progress ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					if( max_len > index ){
						igual_general_file_download_function( index, choosed_demo, revslider, progress );
					}else{
						igual_page_file_download_function( 0, choosed_demo, revslider, progress );
					}
					igual_progress_update( progress );
				},
				error: function(response, errorThrown){
					igual_progress_details( progress, response['msg'], 'failed' );
					index += 1;
	...
	function igual_page_file_download_function( index, choosed_demo, revslider, progress ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					if( max_len > index ){
						igual_page_file_download_function( index, choosed_demo, revslider, progress );
					}else{
						igual_general_file_install_function( 0, choosed_demo, revslider, progress );
					}
					igual_progress_update( progress );
				},
				error: function(response, errorThrown){
		...
		if( max_len == 0 ){
			igual_general_file_install_function( index, choosed_demo, revslider, progress );
		}
		
	}
	
	function igual_general_media_install_function( index, choosed_demo, revslider, progress, parent_index ){
		...
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					index += 1;
					if( max_len > index ){
						igual_general_media_install_function( index, choosed_demo, revslider, progress, parent_index );
					}else{
						igual_progress_update( progress );
						parent_index += 1;
						igual_page_file_install_function( 0, choosed_demo, revslider, progress );
					}
				},
				error: function(response, errorThrown){
					var msg = general_arr[index] + ' not imported';
					igual_progress_details( progress, msg, 'failed' );
					index += 1;
					if( max_len > index ){
						igual_general_media_install_function( index, choosed_demo, revslider, progress, parent_index );
					}else{
	...
	function igual_page_file_install_function( index, choosed_demo, revslider, progress ){
		var parent_index = 0;
		var max_len = page_arr.length;
		if( max_len > index ){
			$.ajax({
				...
				success: function(response){
					...
					index += 1;
					if( max_len > index ){
						igual_page_file_install_function( index, choosed_demo, revslider, progress );
					}else{
						parent_index = media_parts ? 1 : 0;
						igual_general_file_install_function( parent_index, choosed_demo, revslider, progress );
					}
					igual_progress_update( progress );
				},
				error: function(response, errorThrown){
	...
	function igual_general_file_install_function( index, choosed_demo, revslider, progress ){
		var max_len = general_arr.length;
		if( max_len > index ){
		
			if( general_arr[index] == 'media' ){
				igual_general_media_install_function( 0, choosed_demo, revslider, progress, index );
			}else{
				...
				$.ajax({
					...
					success: function(response){
						...
						if( max_len > index ){
							...
							igual_import_final( choosed_demo, progress );
						}
						igual_progress_update( progress );
					},
					error: function(response, errorThrown){
	...
	function igual_import_final( choosed_demo, progress ){
		$.ajax({
			type: 'post',
			dataType: 'json',
			url: ajaxurl,
			...
			success: function(response){
				...
				$.confirm({
					...
					confirmButtonClass: 'btn-success',
					cancelButtonClass: 'btn-danger',
					confirmButton: igual_admin_ajax_var.yes,
					cancelButton: igual_admin_ajax_var.no,
					confirm: function(){
						var win = window.open( igual_admin_ajax_var.regenerate_thumbnails_url , '_blank');
						if (win) {
							win.focus();
						}
						window.location = igual_admin_ajax_var.igual_import_url;
					},
					cancel: function(){
